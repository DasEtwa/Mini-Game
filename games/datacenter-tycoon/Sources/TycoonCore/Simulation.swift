...
public enum CustomerSystem {
    public static func accept(_ id: UUID, serverID: UUID? = nil, in state: inout GameState) throws {
        ...
        request.contract = CustomerContract(months: ContractSystem.term(for: request.typeID), hour: state.hour, priceFactor: state.priceFactor)
    ...
    public static func advance(hours: Int, state: inout GameState) {
        ...
        for _ in 0..<hours {
            ...
            if state.hour % (state.reputation >= 65 ? 36 : 72) == 0 {
                let demand = min(1, 0.30 + (state.reputation-40)/90 + (state.location == .garage ? 0.3 : 0)) / pow(state.priceFactor, 2)
